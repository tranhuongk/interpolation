...
// bounds
Address.prototype.getBounds = function (type) { return this.bounds[type] || this.bounds.default; };
Address.prototype.setBounds = function (bounds, type) {

  // invalid object
  if ('object' !== typeof bounds || null === bounds || !bounds.hasOwnProperty('e') || !bounds.hasOwnProperty('n') || !bounds.hasOwnProperty('s') || !bounds.hasOwnProperty('w')) {
    throw new Error('invalid bounds');
  }

  // parse floats
  bounds.e = parseFloat(bounds.e);
  bounds.n = parseFloat(bounds.n);
  bounds.s = parseFloat(bounds.s);
  bounds.w = parseFloat(bounds.w);

  // ensure the floats are valid
  if (isNaN(bounds.e)) { throw new Error('invalid east'); }
  if (isNaN(bounds.n)) { throw new Error('invalid north'); }
  if (isNaN(bounds.s)) { throw new Error('invalid south'); }
  if (isNaN(bounds.w)) { throw new Error('invalid west'); }

  this.bounds[type || 'default'] = bounds;
